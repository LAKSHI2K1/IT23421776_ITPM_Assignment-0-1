...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "IT23421776_ITPM_Assignment 01.xlsx"),
]
...
def _generate_html_report(ws, header_row: int, input_col_idx: int, expected_col_idx: int, actual_col_idx: int, status_col_idx: int, report_path: str):
    rows = []
    for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
        ...
        if input_value is None or str(input_value).strip() == "":
            continue
        expected_value = _merged_top_left_cell(ws, row_index, expected_col_idx).value if expected_col_idx else None
        actual_value = _merged_top_left_cell(ws, row_index, actual_col_idx).value if actual_col_idx else None
        status_value = _merged_top_left_cell(ws, row_index, status_col_idx).value if status_col_idx else None
        rows.append({
            "row": row_index,
    ...
    pass_count = sum(1 for item in rows if item["status"].upper() == "PASS")
    fail_count = sum(1 for item in rows if item["status"].upper() == "FAIL")
    collected_count = sum(1 for item in rows if item["status"].upper() == "COLLECTED")
    total_count = len(rows)

    header_style = "background:#222;color:#fff;padding:10px;text-align:left;"
    cell_style = "padding:8px;vertical-align:top;border:1px solid #ddd;"
    status_style = {
        "PASS": "color:green;font-weight:bold;",
        "FAIL": "color:red;font-weight:bold;",
        "COLLECTED": "color:orange;font-weight:bold;",
    }

    table_rows = []
    for item in rows:
        style = status_style.get(item["status"].upper(), "")
        table_rows.append(
            f"<tr>"
            f"<td style='{cell_style}'>{item['row']}</td>"
            f"<td style='{cell_style}'>{item['input']}</td>"
            f"<td style='{cell_style}'>{item['expected']}</td>"
            f"<td style='{cell_style}'>{item['actual']}</td>"
            f"<td style='{cell_style}{style}'>{item['status']}</td>"
            f"</tr>"
        )
...
<p><strong>Total cases:</strong> {total_count}</p>
<p><strong>Pass:</strong> {pass_count}</p>
<p><strong>Fail:</strong> {fail_count}</p>
<p><strong>Collected:</strong> {collected_count}</p>
</div>
<table>
<thead>
<tr><th style='{header_style}'>Row</th><th style='{header_style}'>Input</th><th style='{header_style}'>Expected</th><th style='{header_style}'>Actual</th><th style='{header_style}'>Status</th></tr>
</thead>
<tbody>
{''.join(table_rows)}
</tbody>
...
def _parse_args():
    ...
    parser.add_argument("--report", default=None, help="Write an HTML report to this file path")
...
def run_test():
    ...
    args.report = _resolve_path(args.report) if args.report else None
    ...
    if args.report:
        _generate_html_report(ws, header_row, input_col_idx, expected_col_idx, actual_col_idx, status_col_idx, args.report)
